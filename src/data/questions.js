...
const VELOG_QUESTIONS = [
  ...
  { topic: "3주차 - MVC 순서", q: "Spring MVC 요청 흐름에서 클라이언트 요청을 가장 먼저 받는 구성요소는?", options: ["Controller","DispatcherServlet","ViewResolver","Repository"], answer: 1, explain: "PPT/Velog 모두 DispatcherServlet이 Front Controller로서 첫 진입점이라고 설명합니다." },
  { topic: "3주차 - MVC 순서", q: "DispatcherServlet이 HandlerMapping을 조회한 다음 수행하는 작업으로 옳은 것은?", options: ["DB 연결 풀 생성","요청을 처리할 Controller 호출","View를 즉시 렌더링","트랜잭션 종료"], answer: 1, explain: "흐름은 요청 수신 → HandlerMapping 조회 → 해당 Controller 실행입니다." },
  { topic: "3주차 - MVC 순서", q: "Controller가 비즈니스 처리 후 DispatcherServlet에 반환하는 것으로 가장 적절한 것은?", options: ["JDBC Connection","Model 데이터 + View 이름 정보","DDL 스크립트","세션 쿠키"], answer: 1, explain: "Controller는 Model 데이터와 뷰 이름(ModelAndView 또는 동등 정보)을 반환합니다." },
  { topic: "3주차 - MVC 순서", q: "ViewResolver가 담당하는 역할은?", options: ["Entity 스캔","뷰 이름을 실제 View 객체로 변환","SQL 튜닝","트랜잭션 롤백"], answer: 1, explain: "ViewResolver는 Controller가 준 뷰 이름을 실제 렌더링 뷰로 해석합니다." },
  { topic: "3주차 - MVC 순서", q: "MVC 계층 호출 순서로 맞는 것은?", options: ["Controller → Service → Repository → DB","Service → Controller → DB","Repository → Service → Controller","View → Service → DB"], answer: 0, explain: "수업 자료의 기본 흐름은 Controller가 Service를, Service가 Repository를 호출합니다." },
  { topic: "3주차 - 컨테이너 구조", q: "Servlet(Web) Context와 Root Context의 접근 관계로 옳은 것은?", options: ["Root가 Web의 Bean에 접근 가능","Web이 Root의 Bean에 접근 가능","둘 다 서로 접근 불가","둘 다 동일 인스턴스"], answer: 1, explain: "자식(Web) 컨텍스트가 부모(Root) 빈을 참조할 수 있습니다." },
  { topic: "3주차 - 설정", q: "AbstractAnnotationConfigDispatcherServletInitializer에서 Root 설정 클래스를 반환하는 메서드는?", options: ["getServletMappings()","getRootConfigClasses()","getHandlerMappings()","getContextPath()"], answer: 1, explain: "PPT의 초기화 흐름에서 Root 설정은 getRootConfigClasses()가 담당합니다." },
  { topic: "3주차 - 설정", q: "Spring 6/7 전환에서 패키지 네임스페이스 변화로 옳은 것은?", options: ["jakarta.* → javax.*","javax.* → jakarta.*","org.springframework.* → javax.*","변경 없음"], answer: 1, explain: "Jakarta EE 전환으로 javax 네임스페이스가 jakarta로 이동했습니다." },
  { topic: "4주차 - 웹폼 순서", q: "웹폼 처리에서 POST 요청 수신 후, Controller가 유효성 검증 전에 수행하는 단계는?", options: ["DTO(Entity) 즉시 저장","요청 파라미터를 DTO로 Data Binding","ViewResolver 우선 호출","트랜잭션 강제 종료"], answer: 1, explain: "폼 입력값은 먼저 DTO(ProductForm 등)에 바인딩된 뒤 검증됩니다." },
  { topic: "4주차 - 웹폼 순서", q: "BindingResult.hasErrors()가 true인 경우 권장되는 흐름은?", options: ["redirect로 성공 페이지 이동","폼 뷰를 그대로 반환해 오류와 입력값을 표시","DB에 임시 저장","예외를 무조건 던짐"], answer: 1, explain: "검증 실패 시 폼 뷰를 반환해야 에러 메시지와 입력값 유지가 가능합니다." },
  { topic: "4주차 - 웹폼 순서", q: "검증 성공 후 일반적인 처리 순서로 옳은 것은?", options: ["Entity → DTO 변환 후 반환","DTO → Entity 변환 후 저장 → redirect","redirect 후 DTO 생성","Validation 생략 후 저장"], answer: 1, explain: "자료에서 DTO 검증 후 Entity 변환, DB 저장, PRG(redirect) 흐름을 설명합니다." },
  { topic: "4주차 - 검증", q: "BindingResult 파라미터 위치 규칙으로 옳은 것은?", options: ["메서드 가장 마지막","@Valid 대상 파라미터 바로 다음","첫 번째 파라미터","위치 무관"], answer: 1, explain: "PPT에서 BindingResult는 @Valid 바로 뒤에 와야 한다고 명시합니다." },
  { topic: "4주차 - 검증", q: "Thymeleaf의 th:errors가 담당하는 기능은?", options: ["SQL 오류 자동 수정","필드 검증 오류 메시지 출력","라우팅 설정","빈 등록"], answer: 1, explain: "검증 실패 시 필드별 에러 메시지를 화면에 렌더링합니다." },
  { topic: "4주차 - 검증", q: "다음 중 가장 엄격한 문자열 검증 어노테이션은?", options: ["@NotNull","@NotEmpty","@NotBlank","@Size"], answer: 2, explain: "NotBlank는 null/빈문자열/공백문자열까지 모두 차단합니다." },
  { topic: "6.1 - JPA Entity Class", q: "JPA에서 클래스가 DB 테이블과 매핑되는 엔티티임을 선언하는 어노테이션은?", options: ["@Component","@Entity","@Controller","@Configuration"], answer: 1, explain: "PPT 5주차의 Entity Class 섹션 핵심입니다." },
  { topic: "6.1 - JPA Entity Class", q: "Entity 클래스 필수 조건으로 맞는 것은?", options: ["반드시 final 클래스","인자 없는 생성자 필요","모든 필드 public","Serializable 필수"], answer: 1, explain: "수업 자료에서 no-arg constructor를 필수 조건으로 제시합니다." },
  { topic: "6.1 - JPA Entity Class", q: "@Table(name=\"...\")을 생략했을 때 기본 동작으로 옳은 것은?", options: ["엔티티 생성 실패","클래스명을 기준으로 테이블명 매핑","무조건 product 테이블 사용","DB별로 랜덤 생성"], answer: 1, explain: "기본적으로 클래스명을 테이블명으로 사용합니다." },
  { topic: "6.1 - JPA Entity Class", q: "GenerationType.IDENTITY 전략의 핵심 특징은?", options: ["애플리케이션이 키를 미리 계산","DB가 INSERT 시 PK를 생성하고 JPA가 read back","UUID만 허용","SEQUENCE 객체 필수"], answer: 1, explain: "Velog/PPT 모두 저장 시 DB가 키를 생성하고 이후 값을 읽어온다고 설명합니다." },
  { topic: "6.1 - JPA Entity Class", q: "GenerationType.AUTO에 대한 설명으로 옳은 것은?", options: ["항상 IDENTITY와 동일","JPA가 DB Dialect에 맞춰 전략 선택","UUID 전용 전략","deprecated"], answer: 1, explain: "AUTO는 구현체가 DB 방언에 맞는 전략을 자동 선택합니다." },
  { topic: "6.2 - 관계 매핑", q: "Product(N) - Category(1) 관계에서 외래키를 직접 관리하는 Owning Side는 일반적으로?", options: ["@OneToMany 쪽","@ManyToOne + @JoinColumn 쪽","양쪽 동일","View 계층"], answer: 1, explain: "FK는 many 쪽에 두는 구조를 기본으로 설명합니다." },
  { topic: "6.2 - 관계 매핑", q: "Category의 @OneToMany(mappedBy=\"category\")에서 mappedBy의 의미는?", options: ["새 FK 생성","FK 소유 필드가 Product.category임을 지정","즉시 로딩 강제","조인 테이블 생성"], answer: 1, explain: "mappedBy는 역방향(비소유) 측이며 FK 관리 주체를 지정합니다." },
  { topic: "6.2 - 관계 매핑", q: "양방향 연관관계에서 addProduct 같은 helper 메서드를 권장하는 이유는?", options: ["SQL 파싱을 줄이기 위해","양쪽 참조를 동시에 동기화하기 위해","트랜잭션 자동 시작","캐시 비활성화"], answer: 1, explain: "객체 그래프 일관성을 위해 양쪽 필드를 함께 맞춰야 합니다." },
  { topic: "6.2 - 관계 매핑", q: "다음 중 관계 매핑에서 위험한 설정으로 자료에서 경고한 것은?", options: ["@OneToMany + cascade=ALL","@ManyToOne + cascade=REMOVE","@ManyToOne + fetch=LAZY","@ManyToMany + PERSIST"], answer: 1, explain: "부모 공유 엔티티까지 삭제될 수 있어 위험하다고 명시됩니다." },
  { topic: "6.2 - 관계 매핑", q: "@ManyToMany에서 일반 권장 cascade 조합은?", options: ["REMOVE만 사용","ALL 사용","{PERSIST, MERGE} 위주 사용","cascade 사용 금지"], answer: 2, explain: "공유 엔티티 삭제 위험 때문에 REMOVE/ALL은 지양합니다." },
  { topic: "6.2 - 관계 매핑", q: "@ManyToMany 관계의 중간 테이블(product_tag 등)에 대한 설명으로 옳은 것은?", options: ["항상 수동 SQL로만 생성","@JoinTable 설정으로 JPA가 관리 가능","OneToOne에서만 사용","mappedBy가 테이블을 직접 생성"], answer: 1, explain: "자료에서 join table은 JPA가 설정 기반으로 관리함을 설명합니다." },
  { topic: "6.2 - 관계 매핑", q: "@ManyToOne의 FetchType 기본값과 실무 권장으로 옳은 것은?", options: ["기본 LAZY, 그대로 사용","기본 EAGER, 실무는 LAZY 명시 권장","기본 NONE","기본 FETCH JOIN"], answer: 1, explain: "기본 EAGER지만 성능 위해 LAZY를 명시하는 패턴을 권장합니다." },
  { topic: "6.2 - 관계 매핑", q: "JOIN FETCH를 사용하는 핵심 이유는?", options: ["트랜잭션을 제거하기 위해","필요한 연관 엔티티를 한 번에 조회해 N+1/LazyInitializationException 완화","DDL 생성을 위해","Bean 스캔 최적화"], answer: 1, explain: "관계 조회 최적화 목적이며 자료에서 대규모 환경 권장으로 제시됩니다." },
  { topic: "6.3 - Lifecycle / JPQL", q: "new 상태 엔티티에 em.persist(entity)를 호출하면 상태가 어떻게 변하는가?", options: ["Detached","Managed","Removed","Closed"], answer: 1, explain: "persist 호출 이후 영속성 컨텍스트가 추적하는 Managed 상태가 됩니다." },
  { topic: "6.3 - Lifecycle / JPQL", q: "em.clear() 호출의 직접 효과는?", options: ["DB 테이블 삭제","영속성 컨텍스트 비우기(엔티티 detach)","트랜잭션 커밋","스키마 자동 생성"], answer: 1, explain: "clear는 1차 캐시를 비워 엔티티를 detached 상태로 만듭니다." },
  { topic: "6.3 - Lifecycle / JPQL", q: "Detached 상태 엔티티의 필드 값을 변경한 뒤 flush만 호출하면?", options: ["DB에 반영된다","DB 반영되지 않는다","컴파일 에러","자동 merge"], answer: 1, explain: "detach된 객체는 변경 감지 대상이 아니므로 DB에 반영되지 않습니다." },
  { topic: "6.3 - Lifecycle / JPQL", q: "Detached 엔티티 변경사항을 다시 반영하려면 일반적으로 어떤 API를 사용하는가?", options: ["persist","merge","refresh","lock"], answer: 1, explain: "merge는 detached 상태 객체를 다시 관리 상태로 병합합니다." },
  { topic: "6.3 - Lifecycle / JPQL", q: "em.remove(entity) 호출 직후에 대한 설명으로 옳은 것은?", options: ["즉시 DB DELETE 확정","영속성 컨텍스트에 삭제 상태 표시, flush/commit 시 DB 반영","INSERT 수행","트랜잭션 종료"], answer: 1, explain: "삭제 SQL은 flush/commit 시점에 실제 반영됩니다." },
  { topic: "6.3 - Lifecycle / JPQL", q: "JPQL 문법의 기준으로 옳은 것은?", options: ["테이블/컬럼 이름 기준","엔티티 클래스/필드 이름 기준","DB 벤더 함수만 허용","SQL과 완전 동일"], answer: 1, explain: "JPQL은 객체 모델 중심 질의 언어입니다." },
  { topic: "6.3 - Lifecycle / JPQL", q: "Bulk UPDATE/DELETE를 수행한 뒤 em.clear()를 고려해야 하는 이유는?", options: ["메모리 절약만을 위해","영속성 컨텍스트를 우회한 DB 반영으로 캐시-DB 불일치 가능","JPA 명세 위반 방지","DDL 롤백 방지"], answer: 1, explain: "벌크 연산은 PC를 거치지 않아 stale 데이터 문제가 생길 수 있습니다." },
  { topic: "6.4 - 설정 / 트랜잭션", q: "@Transactional(readOnly=true)의 일반 효과로 옳은 것은?", options: ["쓰기 성능 증가","Dirty Checking 부담 감소로 조회 성능 최적화에 유리","DDL 자동 실행","LAZY를 EAGER로 변경"], answer: 1, explain: "Velog 7주차에서 readOnly 조회 최적화 포인트를 강조합니다." },
  { topic: "6.4 - 설정 / 트랜잭션", q: "hibernate.hbm2ddl.auto=none 설정 의미는?", options: ["스키마를 매번 recreate","엔티티 기반 DDL 자동 실행 안 함","모든 쿼리 차단","DDL만 허용"], answer: 1, explain: "자료에서 운영/수업 설정으로 자동 테이블 생성 비활성화를 설명합니다." },
  { topic: "6.4 - 설정 / 트랜잭션", q: "트랜잭션 경계 내 메서드 실행의 일반 흐름으로 옳은 것은?", options: ["commit → method → begin","begin → method → commit(또는 rollback)","method만 실행","begin 없이 auto commit"], answer: 1, explain: "@Transactional 기본 동작 설명과 일치합니다." },
  { topic: "2주차 - 현대적 스프링 진화", q: "Virtual Thread가 특히 유리한 시나리오는?", options: ["CPU 바운드 연산 집중","I/O 블로킹이 많은 고동시성 요청 처리","단일 스레드 배치","JVM 없이 실행"], answer: 1, explain: "강의/Velog 모두 I/O 중심 애플리케이션에서 장점을 강조합니다." },
  { topic: "2주차 - 현대적 스프링 진화", q: "GraalVM Native Image(AOT)의 대표 효과로 옳은 것은?", options: ["느린 시작 속도","빠른 시작 시간과 낮은 메모리 풋프린트","JIT 의존도 증가","ORM 비활성화"], answer: 1, explain: "배포 진화 파트에서 Instant Startup/Small Footprint가 핵심으로 제시됩니다." },
  { topic: "2주차 - 현대적 스프링 진화", q: "Spring AI에서 PSA를 강조하는 이유로 가장 적절한 것은?", options: ["AI 모델 하나만 고정 사용","모델 공급자 교체/확장 시 애플리케이션 코드 변경을 최소화","DB 트랜잭션 처리","서블릿 버전 업그레이드"], answer: 1, explain: "공통 인터페이스 추상화로 OpenAI/기타 모델 교체를 쉽게 하기 위함입니다." },
  { topic: "2주차 - 현대적 스프링 진화", q: "멀티스레드 톰캣 환경에서 자료가 특히 강조한 키워드는?", options: ["메모리 압축","상호배제(Mutual Exclusion)","코드 난독화","직렬화 제거"], answer: 1, explain: "공유 자원 접근 안전성을 위해 상호배제를 반복 강조했습니다." },
  { topic: "1주차 - 스프링 개요 / 프레임워크 기본", q: "다음 중 WAS의 설명으로 가장 적절한 것은?", options: ["정적 파일만 전달한다","동적 요청 처리와 DB 연동을 담당한다","브라우저에서만 동작한다","Java 없이도 Spring MVC를 실행한다"], answer: 1, explain: "1주차 자료에서 WAS는 동적 페이지 처리와 DB 통신을 담당한다고 설명합니다." },
  { topic: "1주차 - 스프링 개요 / 프레임워크 기본", q: "웹에서 클라이언트-서버가 주로 사용하는 통신 규약은?", options: ["FTP","SMTP","HTTP","JDBC"], answer: 2, explain: "Velog 1주차 설명에서 HTTP 프로토콜 기반 통신을 명시합니다." },
  { topic: "1주차 - 스프링 개요 / 프레임워크 기본", q: "프레임워크를 쓰는 주된 이유로 옳은 것은?", options: ["DB 스키마를 자동 완성하기 위해","구조와 기반 코드를 제공해 핵심 로직에 집중하기 위해","코드를 전혀 작성하지 않기 위해","컴파일 속도를 강제로 높이기 위해"], answer: 1, explain: "구조 품질(유지보수성, 재사용성, 확장성)을 위해 프레임워크를 사용합니다." },
  { topic: "2주차 - 현대적 스프링 진화", q: "기존 스레드가 대규모 요청에서 부담이 커지는 직접적 이유로 자료에서 강조한 것은?", options: ["스레드마다 스택 메모리 할당 비용이 크기 때문","스레드가 DB 연결을 자동 닫기 때문","HTTP가 스레드를 직접 생성하기 때문","톰캣이 단일 스레드만 지원하기 때문"], answer: 0, explain: "요청 수 증가 시 스레드 수만큼 스택 메모리 비용이 커진다는 점을 강조했습니다." },
  { topic: "2주차 - 현대적 스프링 진화", q: "Thread Pool 방식의 대표적 한계로 옳은 것은?", options: ["미리 만든 스레드 수를 초과한 요청은 대기한다","요청마다 스레드가 무한히 생성된다","I/O 작업을 전혀 처리하지 못한다","트랜잭션을 사용할 수 없다"], answer: 0, explain: "2주차 정리에서 pool 크기 초과 시 대기열이 생기는 한계를 언급했습니다." },
  { topic: "2주차 - 현대적 스프링 진화", q: "운영의 진화(Cloud Native & Observability)에서 관측성의 핵심 예시는?", options: ["JSP 자동 컴파일","Metrics/Tracing 수집","XML 제거","정적 라우팅"], answer: 1, explain: "비즈니스 로직과 분리된 모니터링 지표 수집이 핵심으로 설명됩니다." },
  { topic: "2주차 - DI / Bean", q: "아래 코드의 핵심 의미는?\n\npublic PetOwner(AnimalType animal) { this.animal = animal; }", options: ["Dog 구현체를 고정 사용한다","생성자 주입으로 인터페이스에 의존한다","필드 주입만 허용한다","DI 없이 수동 결합을 강제한다"], answer: 1, explain: "구현체가 아닌 인터페이스를 주입받아 결합도를 낮추는 패턴입니다." },
  { topic: "2주차 - DI / Bean", q: "싱글톤 Bean에서 특히 피해야 할 것으로 수업 자료가 강조한 것은?", options: ["불변 상수 선언","가변(mutable) 멤버 필드의 공유","인터페이스 구현","생성자 사용"], answer: 1, explain: "공유 상태와 상호배제 이슈 때문에 mutable 멤버 변수 사용을 주의해야 합니다." },
  { topic: "2주차 - DI / Bean", q: "Spring Container의 핵심 역할이 아닌 것은?", options: ["Bean 생성/관리","의존성 주입","객체 생명주기 관리","브라우저 렌더링"], answer: 3, explain: "렌더링은 웹 계층(View)의 책임이며 컨테이너 핵심 역할이 아닙니다." },
  { topic: "3주차 - MVC", q: "다음 중 Web(자식) 컨텍스트에 주로 등록되는 Bean은?", options: ["Controller, ViewResolver 등 웹 계층","DataSource, Repository 등 데이터 계층","JVM 옵션 설정","OS 스케줄러"], answer: 0, explain: "수업 자료는 Root와 Web 컨텍스트 역할 분리를 명확히 설명합니다." },
  { topic: "3주차 - MVC", q: "HandlerMapping이 Controller를 찾기 위해 참고하는 대표 정보는?", options: ["CPU 코어 수","요청 URL과 매핑 어노테이션","DDL 로그","GC 횟수"], answer: 1, explain: "@RequestMapping/@GetMapping 등의 매핑 정보를 기준으로 핸들러를 찾습니다." },
  { topic: "3주차 - MVC", q: "다음 중 Spring MVC 실행 순서로 옳은 것은?", options: ["ViewResolver → DispatcherServlet → Controller","DispatcherServlet → HandlerMapping → Controller → ViewResolver","Controller → DispatcherServlet → DB","DB → Repository → DispatcherServlet"], answer: 1, explain: "Front Controller 패턴의 표준 흐름과 일치합니다." },
  { topic: "4주차 - 웹폼 / 검증", q: "GET 방식이 검색/필터에 자주 쓰이는 이유로 적절한 것은?", options: ["본문(body)에만 데이터가 담겨서","URL 기반으로 북마크/재호출이 용이해서","암호화가 기본이라서","서버 부하가 0이라서"], answer: 1, explain: "GET은 쿼리스트링 기반이라 재방문/공유가 쉬운 특성이 있습니다." },
  { topic: "4주차 - 웹폼 / 검증", q: "POST 방식이 Create/Update 폼에 적합한 이유로 옳은 것은?", options: ["파라미터가 URL에 그대로 노출된다","요청 본문으로 데이터 전달이 가능하다","반드시 캐시된다","검증이 불가능하다"], answer: 1, explain: "POST는 요청 본문 사용으로 데이터 노출을 줄이고 폼 제출에 적합합니다." },
  { topic: "4주차 - 웹폼 / 검증", q: "Data Buffering의 목적은?", options: ["DB 연결을 줄이기 위해","검증 실패 후에도 사용자가 입력한 값을 폼에 유지하기 위해","컨테이너 재시작을 빠르게 하기 위해","트랜잭션을 강제하기 위해"], answer: 1, explain: "검증 실패 시 사용자 입력값을 유지해 수정 재제출 UX를 제공합니다." },
  { topic: "5주차 - JPA 기본", q: "JPA / Hibernate / Spring Data JPA 관계로 가장 정확한 것은?", options: ["JPA는 구현체, Hibernate는 명세","JPA는 명세, Hibernate는 구현체, Spring Data JPA는 상위 편의 모듈","세 기술은 완전히 독립","Spring Data JPA만 쓰면 JPA 지식이 필요 없다"], answer: 1, explain: "PPT/Velog 모두 세 기술의 계층적 관계를 동일하게 설명합니다." },
  { topic: "5주차 - JPA 기본", q: "EntityManagerFactory와 EntityManager 관계 설명으로 옳은 것은?", options: ["EMF가 트랜잭션마다 생성되고 EM은 앱 당 1개","EMF는 앱 시작 시 생성되고, EM은 트랜잭션/요청 단위로 사용","둘 다 스레드 안전하지 않다","둘 다 DB 세션과 무관하다"], answer: 1, explain: "EMF는 상대적으로 무겁고 공유되며, EM은 단위 작업 범위에서 사용됩니다." },
  { topic: "5주차 - JPA 기본", q: "Persistence Context의 기능으로 옳지 않은 것은?", options: ["1차 캐시","변경 감지(Dirty Checking)","지연 SQL 실행(Write-behind)","View 템플릿 렌더링"], answer: 3, explain: "렌더링은 웹 계층 책임이며 영속성 컨텍스트 기능이 아닙니다." },
  { topic: "6.1 - JPA Entity Class", q: "다음 중 Entity 클래스 설명으로 옳지 않은 것은?", options: ["@Entity로 영속 대상임을 명시한다","무인자 생성자가 필요하다","@Id는 PK를 나타낸다","@Entity 없이도 JPA가 자동으로 영속 클래스로 인식한다"], answer: 3, explain: "영속 엔티티 인식을 위해 @Entity 선언이 필요합니다." },
  { topic: "6.2 - 관계 매핑", q: "6.2에서 Category-Product 양방향 매핑 시 Category 쪽에 mappedBy를 두는 직접 이유는?", options: ["새로운 FK를 생성하기 위해","FK를 관리하는 소유 필드가 Product 쪽임을 알려주기 위해","EAGER 로딩을 강제하기 위해","조인 테이블 생성 방지를 위해"], answer: 1, explain: "mappedBy는 비소유 측 선언으로 FK 관리 주체를 명시합니다." },
  { topic: "6.2 - 관계 매핑", q: "다음 중 cascade 사용 가이드로 자료와 일치하는 것은?", options: ["ManyToMany에 cascade=REMOVE 적극 권장","ManyToOne에서 cascade=REMOVE는 위험","OneToMany에서는 cascade 사용 금지","모든 관계에서 cascade=ALL 고정"], answer: 1, explain: "공유 참조 삭제 전파 위험 때문에 ManyToOne/ManyToMany REMOVE를 경고합니다." },
  { topic: "6.3 - Lifecycle / JPQL", q: "persist → flush → clear 이후 같은 ID로 find를 호출하면 일반적으로 어디에서 조회되는가?", options: ["항상 1차 캐시","DB에서 다시 조회","2차 캐시만 조회","조회 불가"], answer: 1, explain: "clear로 1차 캐시가 비워졌으므로 이후 find는 DB 조회가 발생합니다." },
  { topic: "6.3 - Lifecycle / JPQL", q: "persist와 merge의 차이로 가장 적절한 것은?", options: ["persist는 기존 detached 병합, merge는 신규 저장","persist는 신규 영속화, merge는 detached 상태 병합에 주로 사용","둘은 완전히 동일","merge는 삭제 전용"], answer: 1, explain: "자료에서 persist(신규)와 merge(병합) 용도를 구분해 설명합니다." },
  { topic: "6.4 - 설정 / 트랜잭션", q: "조회 전용 서비스 메서드에 @Transactional(readOnly=true)를 두는 이유로 적절한 것은?", options: ["모든 연산을 자동 롤백","변경 감지 부담을 줄여 조회 성능에 유리","항상 EAGER 로딩으로 바꾸기 위해","JPA 비활성화"], answer: 1, explain: "읽기 전용 힌트로 불필요한 변경 감시 비용을 낮출 수 있습니다." },
  { topic: "6.4 - 설정 / 트랜잭션", q: "hbm2ddl.auto 설정을 운영에서 무분별하게 create/create-drop으로 두면 생길 수 있는 리스크는?", options: ["뷰 렌더링 지연","스키마가 의도치 않게 재생성/삭제될 수 있음","DI 실패","HTTP 통신 실패"], answer: 1, explain: "자료에서 none/validate 등 안전한 설정 사용 중요성을 강조합니다." }
];
